...
"use strict";
/**
 * Dual Calendar v2 — Umm‑al‑Qura aligned by default + Fri/Sat weekend highlight
 * - Offline-first (tabular Islamic calendar); one-shot refine via Aladhan gToH
 * - If API is unavailable, align locally to anchor: 1 Ramadan 1447 = 18 Feb 2026 (Umm‑al‑Qura)
 * - No alerts; very low CPU; event delegation
 */
...
const state = {
  ...
  hOffsetMonths: 0,
  refineOffsetDays: 0,
  refineEnabled: true, // default ON in v2
};
...
const ISLAMIC_EPOCH = 1948439.5; // JD of 1 Muharram 1 AH
...
// ---- Init ----
function init(){
  // Default refine ON; respect prior choice
  const saved = localStorage.getItem('refineEnabled');
  if (saved!==null) state.refineEnabled = (saved==='1');
  ...
  const {hYear,hMonth}=gToH(y,m,d); state.hYear=hYear; state.hMonth=hMonth;
  ...
  dom.useApiToggle.addEventListener('change', async (e)=>{
    state.refineEnabled = !!e.target.checked; localStorage.setItem('refineEnabled', state.refineEnabled?'1':'0');
    if (state.refineEnabled) { await refineWithApiOnce().catch(()=>localUmmAlQuraFallbackAdjust()); } else { state.refineOffsetDays=0; dom.apiStatus.textContent='offline mode'; localUmmAlQuraFallbackAdjust(); rerenderAll(); }
  });
...
function renderGregorian(){
  ...
  buildGrid(dom.gGrid, firstDow, daysInMonth, (day)=>{ const h=gToH(year,month,day); return { gLabel:String(day), hLabel:`${h.hDay} ${HIJRI_MONTHS[h.hMonth-1]}`, meta:{y:year,m:month,d:day} }; }, 'greg');
}
...
function renderHijri(){
  ...
  buildGrid(dom.hGrid, firstDow, daysIn, (day)=>{ const g=hToG(y,m,day); return { gLabel:`${g.day}/${g.month}`, hLabel:String(day), meta:{hy:y,hm:m,hd:day} }; }, 'hijri');
}
...
function buildGrid(container, firstDow, daysInMonth, makeCell, mode){
  ...
  for(let day=1; day<=daysInMonth; day++){
    ...
    // Weekend highlight for Fri (5) & Sat (6)
    ...
    cell.addEventListener('click', ()=>{
      ...
      if(mode==='greg'){
        ...
        showSelection({ g:`${FMT_GREG_DATE.format(gDate)}`, h:`${h.hDay} ${HIJRI_MONTHS[h.hMonth-1]} ${h.hYear} AH` });
      }else{
        const g=hToG(data.meta.hy,data.meta.hm,data.meta.hd); const gDate=new Date(g.year,g.month-1,g.day);
        showSelection({ g:`${FMT_GREG_DATE.format(gDate)}`, h:`${data.meta.hd} ${HIJRI_MONTHS[data.meta.hm-1]} ${data.meta.hy} AH` });
      }
    });
...
function renderTodayBlock(){ const now=new Date(); const y=now.getFullYear(), m=now.getMonth()+1, d=now.getDate(); const h=gToH(y,m,d); dom.todayBlock.innerHTML = `<div><strong>Gregorian today:</strong> ${FMT_GREG_DATE.format(now)}</div><div><strong>Hijri today:</strong> ${h.hDay} ${HIJRI_MONTHS[h.hMonth-1]} ${h.hYear} AH</div>`; }
...
// ---- One-shot API refine (Umm‑al‑Qura via Aladhan) ----
async function refineWithApiOnce(){
  try{
    dom.apiStatus.textContent='refine: contacting…';
    const now=new Date(); const y=now.getFullYear(), m=now.getMonth()+1, d=now.getDate();
    const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(), 5000);
    const res=await fetch(`https://api.aladhan.com/v1/gToH/${d}-${m}-${y}?method=4`, { signal: controller.signal });
    clearTimeout(timer);
    if(!res.ok) throw new Error('HTTP '+res.status);
    const json=await res.json();
    if(json && json.code===200 && json.data && json.data.hijri){
      const h=json.data.hijri; const target={ y:parseInt(h.year,10), m:parseInt(h.month.number,10), d:parseInt(h.day,10) };
      const jdToday=gregorianToJD(y,m,d); let best=0, found=false;
      for(let off=-2; off<=2; off++){ const probe=jdToIslamic(jdToday+off+state.refineOffsetDays); if(probe.hYear===target.y && probe.hMonth===target.m && probe.hDay===target.d){ best=off; found=true; break; } }
      state.refineOffsetDays = found ? (state.refineOffsetDays+best) : state.refineOffsetDays;
      dom.apiStatus.textContent = found ? `refined (offset ${best}d)` : 'refine: no change';
      rerenderAll();
      return;
    }
    throw new Error('Bad payload');
  }catch(e){ dom.apiStatus.textContent='refine failed (offline)'; throw e; }
}

// ---- Local fallback alignment (anchor to Umm‑al‑Qura) ----
function localUmmAlQuraFallbackAdjust(){
  // Anchor: 1 Ramadan 1447 AH == 18-Feb-2026 (Umm‑al‑Qura)
  const anchorG={ year:2026, month:2, day:18 }; const anchorH={ y:1447, m:9, d:1 };
  try{
    const jdA=gregorianToJD(anchorG.year, anchorG.month, anchorG.day);
    let best=0, found=false; for(let off=-2; off<=2; off++){ const probe=jdToIslamic(jdA+off+state.refineOffsetDays); if(probe.hYear===anchorH.y && probe.hMonth===anchorH.m && probe.hDay===anchorH.d){ best=off; found=true; break; } }
    if(found){ state.refineOffsetDays += best; dom.apiStatus.textContent=`refined locally (offset ${best}d)`; rerenderAll(); }
  }catch{ /* ignore */ }
}
